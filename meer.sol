...
library CheckMeerSigs {
    ...
    function hash160(bytes memory _data) internal view returns (bytes20) {
        bytes29 _pubkeyView = _data.ref(0);
        return _pubkeyView.hash160();
    }
    ...
    function p2wpkhFromPubkey(bytes memory _pubkey) internal view returns (bytes memory) {
        ...
        bytes20 _digest;
        if (_pubkey.length == 32) {_digest = _pubkeyView.hash160();}
        ...
        return abi.encodePacked( _digest);
    }
